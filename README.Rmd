...
The model has been calibrated for `r nrow(bf_params_usgs)` selected gaging sites. Parameters are available for the USGS gage parameters dataset `bf_params_usgs` included with this package, as excerpted below.  Description of the parameters in this dataset can be viewed by calling the help `r ?bf_params_usgs`.
...
`bfsep` works best in conjuction with the USGS `dataRetrieval` package to fetch time-series data from specific gages.
